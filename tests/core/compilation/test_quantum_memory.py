...
class TestQuantumMemory:
    def test_allocate_qubits_to_node_grows_capacity_and_assigns_slots(
        self, memory_nodes: list[VariableNode]
    ) -> None:
        memory = QuantumMemory()

        slots = memory.allocate_qubits_to_node(memory_nodes[:2])
        allocation = memory.node_allocation_snapshot(set(memory_nodes[:2]))

        assert len(slots) == 2
        assert all(isinstance(slot, PhysicalQubit) for slot in slots)
        assert memory.size == 2
        assert {slot.integer_index for slot in slots} == {0, 1}
        assert {slot.integer_index for slot in memory.slots} == {0, 1}
    ...
    def test_allocate_qubits_to_node_reuses_freed_slots_before_growing(
        self, memory_nodes: list[VariableNode]
    ) -> None:
        memory = QuantumMemory()
        initial_slots = memory.allocate_qubits_to_node(memory_nodes[:2])
        memory.free_nodes([memory_nodes[0]])
        ...
        assert memory.size == 2
        assert (
            memory.node_allocation_snapshot(
    ...
    def test_allocate_qubits_to_node_raises_when_request_exceeds_available_capacity(
        self, memory_nodes: list[VariableNode]
    ) -> None:
        memory = QuantumMemory(size_limit=2)
        memory.allocate_qubits_to_node(memory_nodes[:2])
        memory.free_nodes([memory_nodes[1]])

        with pytest.raises(RuntimeError, match="Not enough free qubits"):
            memory.allocate_qubits_to_node(memory_nodes[2:4])

        assert memory.size == 2
        assert not memory.is_allocated(memory_nodes[2])
        assert not memory.is_allocated(memory_nodes[3])

    def test_free_slots_releases_slots_and_updates_allocation(
        self, memory_nodes: list[VariableNode]
    ) -> None:
        memory = QuantumMemory()
        allocated_qubits = memory.allocate_qubits_to_node(memory_nodes[:3])

        result = memory.free_nodes(memory_nodes[:2])
    ...
    def test_swap_data_qubits_exchanges_physical_assignments(
        ...
        memory = QuantumMemory()
        allocated = memory.allocate_qubits_to_node(memory_nodes[:2])
    ...
    def test_swap_data_qubits_raises_for_unallocated_qubit(
        ...
        memory = QuantumMemory()
        memory.allocate_qubits_to_node([memory_nodes[0]])

        with pytest.raises(RuntimeError, match="is not currently allocated"):
            memory.swap_data_qubits(memory_nodes[0], memory_nodes[1])

    def test_free_nodes_raises_for_unallocated_qubit(
        self, memory_nodes: list[VariableNode]
    ) -> None:
        memory = QuantumMemory()
    ...
    def test_node_allocation_snapshot_exposes_read_only_view(
        self, memory_nodes: list[VariableNode]
    ) -> None:
        memory = QuantumMemory()
        allocated_qubit = memory.allocate_qubits_to_node([memory_nodes[0]])[0]

        allocation = memory.node_allocation_snapshot(
