...
class LRCircuit(PrimitiveImplementation[ExtractSyndrome]):
    ...
    This builds a Stim circuit with the package's documented LR-circuit workflow,
    converts the generated Stim operations into EPIC instruction lines, creates
    measurements from the generated ancilla measurements, and forms detectors
    following the same pattern as the RSC syndrome-extraction primitive.
    ...
    def compile(
        ...
        measurement_ops = {"M", "MX", "MZ", "MY", "MR", "MRX", "MRY", "MRZ"}
        ...
        for instr in circuit:
            ...
            if not indices:
                stim_instructions.append(instr)
                continue

            nodes = [index_to_node[int(idx)] for idx in indices]
            qubits = [str(node_to_qubit[node].integer_index) for node in nodes]
            stim_instructions.append(f"{op} {' '.join(qubits)}")
